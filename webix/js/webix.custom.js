	...
	defaults : {
		...
		on : {
			...
			onAfterEditStop : function(state, editor, ignore) {
				console.log("onAfterEditStop", state, editor, ignore);
				...
				if(record["rowStatus"] != "C" && oldVal != newVal){
					record["rowStatus"] = "U";
				}
				...
				if (evt && evt.type == "click" && evt.srcElement.tagName == "BUTTON" && $(evt.srcElement).hasClass("btnCodeHelpGrid")) {
					...
					if (option.type == "code") {
						...
						var callback = function(result) {
							...
							if(record["rowStatus"] != "C" && oldVal != (isEmpty(result) ? "" : result["CODE"])){
								record["rowStatus"] = "U";
							}
				...
				if(column.option&& column.option.type == "code"){
					var value = editor.getValue();
			...
			onLiveEdit : function(state, editor, keyCode) {
				...
				if (option.type == "code") {
					...
					if (record["rowStatus"] != "C" && state.old != state.value) {
						record["rowStatus"] = "U";
					}
			...
			onItemClick : function(target, evt, html) {
				...
				var gridID = this.config.id; //gridID
				var text = this.getText(target.row, target.column); // text (TODO: display text);
				var value = record[target.column]; // value
				var rowIndex = this.getIndexById(target.row); // rowIndex;
				var dataField = target.column; // dataField;
				var rowData = record; // rowData;
				var row = record; // row (TODO: row information);
				var column;
				var columnIndex;
				...
				if (columnConfig.editor === "button") {
					console.log(columnConfig.option, columnConfig.option.callBackFn);
...
//		if (!view || !view._custom_tab_handler && !view._custom_tab_handler(true, evt)) {
		...
		if ((option.type == "code"
		  || option.type == "sidossouser"
		  || option.type == "cmuser"
		  || option.type == "prcuserwork"
		  || option.type == "sidoorg"
			  ) && check) {
...
var dxGrid = {
	initGrid: function (gridID, width, height, columns, option) {
		...
		if (option.checkbox === true) {
			cols.push({id:"__CHK", header:{ content:"masterCheckbox", contentId:"mc1" }, css:"textCenter", width: 40, template:"{common.checkbox()}", tooltip : false});
		}
