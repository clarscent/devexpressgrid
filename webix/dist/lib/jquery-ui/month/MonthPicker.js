...
(function($, window, document, undefined) {
	...
	$.widget("KidSysco.MonthPicker", {
		...
		_destroy : function() {
			...
			this.element.val('').css('color', '').removeClass('month-year-input').removeData(this._enum._overrideStartYear).unbind();
		...
		_init : function() {
			...
			this._monthPickerMenu = $('<div id="MonthPicker_' + this.element.attr('id') + '" class="month-picker ui-helper-clearfix" style="z-index:999999;"></div>');
			...
			$('.previous-year button', this._monthPickerMenu).button({
				icons : {
			...
			$('.next-year button', this._monthPickerMenu).button({
				icons : {
		...
		GetSelectedMonthYear : function() {
			var _month = this._validateMonth(this.element.val()), _year = this._validateYear(this.element.val()), _date;
		...
		_showIcon : function() {
			if (this._monthPickerButton === null) {
				if (this.options.ShowIcon) {
					this._monthPickerButton = $('<button id="MonthPicker_Button_' + this.element.attr('id') + '" class="month-picker-open-button"></button>').insertAfter(this.element);
					this._monthPickerButton.click($.proxy(this._show, this));
				} else {
		...
		_show : function() {
			...
			if (this._monthPickerMenu.css('display') === 'none') {
				...
				// 178  높이 초과할 경우 위로 올림;
				if (_top + 178 > $(document).height()) {
					_top = $(document).height() - 178;
				}
				
				console.log($(document).height(), this.element, this.element.height(), _top);
		...
		_setDisabledState : function() {
			if (this.options.Disabled) {
				...
				if (this._monthPickerButton !== null) {
					this._monthPickerButton.attr('disabled', true);
				}
				...
				if (this._monthPickerButton !== null) {
					this._monthPickerButton.attr('disabled', false);
				}
			}
		},
		...
		_chooseMonth : function(month) {
			...
			if (this._isFunction(this.options.OnAfterChooseMonth)) {
				this.options.OnAfterChooseMonth(this.element, month);
			}
			
			
			// KJH Change
			try {
				if (oldValue != this.element.val()) {
					listener.select.change(this.element);
				}
			} catch (e) {
				console.log("month picker custom event error", e);
			}
		},
		...
		_showMonths : function() {
			...
			$('.previous-year button', this._monthPickerMenu).attr('title', this._i18n('prevYear')).unbind('click').bind('click.MonthPicker', $.proxy(this._previousYear, this));

			$('.next-year button', this._monthPickerMenu).attr('title', this._i18n('nextYear')).unbind('click').bind('click.MonthPicker', $.proxy(this._nextYear, this));
			...
			for ( var _month in _months) {
				...
				$('.button-' + _counter, this._monthPickerMenu).bind('click.MonthPicker', {
					_month : _counter
		...
		_showYears : function() {
			...
			$('.previous-year button', this._monthPickerMenu).attr('title', this._i18n('prev5Years')).unbind('click').bind('click', $.proxy(function() {
				this._previousYears();
				return false;
			}, this));

			$('.next-year button', this._monthPickerMenu).attr('title', this._i18n('next5Years')).unbind('click').bind('click', $.proxy(function() {
				this._nextYears();
				return false;
			}, this));
