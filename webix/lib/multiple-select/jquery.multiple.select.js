...
(function($) {
	...
	function MultipleSelect($el, options) {
		...
		this.$choice = $('<button type="button" class="ms-choice" data-select-id="' + $el.attr("id") + '"><span class="placeholder">' + options.placeholder + '</span><div></div></button>');
	...
	MultipleSelect.prototype = {
		...
		init : function() {
			...
			if (this.options.selectAll && !this.options.single) {
				ul.append('<li class="ms-select-all">' + '<input type="checkbox" ' + this.selectAllName + ' id="' + this.$el.attr('id') + '_all" /> ' + '<label for="' + this.$el.attr('id') + '_all">' + this.options.selectAllDelimiter[0] + this.options.selectAllText + this.options.selectAllDelimiter[1] + '</label>' + '</li>');
			}
		...
		optionToHtml : function(i, elm, group, groupDisabled) {
			var that = this, $elm = $(elm), multiple = this.options.multiple, optAttributesToCopy = [
					'class', 'title'
			...
			if ($elm.is('option')) {
				...
				if ((this.options.blockSeparator > "") && (this.options.blockSeparator == $elm.val())) {
					var li = $('<li' + clss + style + '>', '<label class="' + this.options.blockSeparator + (disabled ? 'disabled' : '') + '" for="' + this.$el.attr('id') + '_' + value + '">', '</label>', '</li>');
					li.find('label').append(document.createTextNode(text));
					return li;
				} else {
					// 현재 여기
					var li = $('<li' + clss + style + '>' + '<input type="' + type + '" ' + this.selectItemName + (selected ? ' checked="checked"' : '') + (disabled ? ' disabled="disabled"' : '') + (group ? ' data-group="' + group + '"' : '') + ' id="' + this.$el.attr('id') + '_' + value + '" /> ' + '<label' + (disabled ? ' class="disabled"' : '') + ' for="' + this.$el.attr('id') + '_' + value + '">' + '</label>' + '</li>');
				...
				var group = $('<div/>');
				group.append('<li class="group">' + '<label class="optgroup' + (disabled ? ' disabled' : '') + '" data-group="' + _group + '" for="' + this.$el.attr('id') + '_' + value + '">' + (this.options.hideOptgroupCheckboxes ? '' : '<input type="checkbox" ' + this.selectGroupName + (disabled ? ' disabled="disabled"' : '') + ' id="' + this.$el.attr('id') + '_' + value + '" /> ') + label + '</label>' + '</li>');
				li.find('label').append(document.createTextNode(text));
		...
		events : function() {
			...
			this.$choice.off('keydown').on('keydown', function(evt) {
			...
			this.$searchInput.off('keydown').on('keydown', function(e) {
				...
				if (that.options.filterAcceptOnEnter && (e.which === 13 || e.which == 32) && // enter or space
			...
			this.$selectAll.off('click').on('click', function() {
				var checked = $(this).prop('checked'), $items = that.$selectItems.filter(':enabled');
		...
		update : function(isInit) {
			...
			} else if (this.options.countSelected && selects.length < this.options.minimumCountSelected && !(selects.length === this.$selectItems.length + this.$disableItems.length)) {
				$span.removeClass('placeholder').text((this.options.displayValues ? selects : this.getSelects('text')).join(this.options.delimiter));
			} else if (!this.options.single && this.options.allSelected && selects.length === this.$selectItems.length + this.$disableItems.length) {
	...
	$.fn.multipleSelect = function() {
		...
		value, allowedMethods = [
				'getSelects', 'setSelects', 'enable', 'disable', 'checkAll', 'uncheckAll', 'focus', 'blur', 'refresh', 'close', 'clearAll', 'init', 'isEnable'
		];
