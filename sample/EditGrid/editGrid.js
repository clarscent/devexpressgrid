...
const Grid = {
    ...
    method: {
         ...
         *
         */
        addRow : async function (gridId, data, index) {
            let instance = Grid.method.getGridInstance(gridId);
            ...
            instance.refresh().done(() => {
                instance.option("focusedRowEnabled", true);
                Grid.method.__setFocusOnFirstCellInRow(instance, index);
            });

        },

        /**
         * 열의 첫 번째 행에 포커스를 두는 함수
         * @param instance  - grid instance
         * @param index     - rowIndex
         * @private
         */
        __setFocusOnFirstCellInRow : function (instance, index) {
            let firstColumn = instance.option('columns')[0].dataField;
            let cellElement = instance.getCellElement(index, firstColumn);
        ...
        __addEventListener : function (gridId) {
            ...
            instance.option("onRowInserting", function(eventObject) {
                Grid.method.__executeListener("onRowInserting", eventObject, gridId, eventObject.component, eventObject.element, eventObject.model, eventObject.data, eventObject.cancel);
            });
...
let Listener = {
    grid : {
        ...
        onRowInserting : function (gridId, component, element, model, data, cancel) { },
